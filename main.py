LICENCE = """
Copyright © 2022 Azury#2022
Ce logiciel a été developper entièrement par Azury (ps: c"est mon premier tool).
...
class NitroGen: # Initialiser la classe
    ...
    def main(self): # La fonction principale contient le code le plus important
        ...
        if os.name == "nt": # Si le système est Windows
            print("")
            ctypes.windll.kernel32.SetConsoleTitleW("Azury | Nitro Generator et Checker") # Changer le
        else: # Ou si c'est unix
            print(f'\33]0;Azury | Nitro Generator et Checker\a', end='', flush=True) # Mettre à jour le titre du cmd
 ...
 /_/   \_\/___|\__,_|_|   \__, |
        ...
        \/     \/                   
          Par Azury#2022
    Rejoint le discord ! https://discord.gg/lightgen         
        ...
        for i in range(num): # Boucle sur le nombre de codes à vérifier
            ...
            if os.name == "nt": # Si le système est Windows
                ctypes.windll.kernel32.SetConsoleTitleW(f"Azury | Nitro Generator et Checker - {len(valide)} Valide | {invalide} Invalide ") # Modifier le titre du cmd
                print("")
            else: # S'il s'agit d'un système unix
                print(f'\33]0;Nitro Generator et Checker - {len(valide)} Valide | {invalide} InValide \a', end='', flush=True) # Modifier le titre du cmd
